    ...
    return(
        ...
        <nav className="flex sm:justify-between justify-center sm:gap-0 gap-4 p-4 border-b-1 flex-wrap border-main-2 bg-backgroundColors-1">
            ...
            {!Cookies.get('accessToken') && <div className="flex gap-4">
                ...
                <Link className="bg-main-1 text-white min-w-28 flex items-center justify-center rounded-lg" href={'/register'}>Sign Up</Link>

                {/* <BaseButton extraClass="min-w-28">Sign In</BaseButton>
                <BaseButton extraClass="min-w-28">Sign Up</BaseButton> */}
            </div> }
            {
                Cookies.get('accessToken') && <BaseButton isLoading={logoutMutation.isLoading} isDisabled={logoutMutation.isLoading} onClick={()=>logoutMutation.mutate()} extraClass="min-w-28">Log Out</BaseButton>
            }
